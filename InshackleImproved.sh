...
printf "      \e[1;31m[\e[0m\e[1;77mv1.0\e[0m\e[1;31m]\e[0m\e[1;77m github.com/DiamondLink/InshackleImproved (original by @thelinuxchoice)\e[0m\n"
}
...
if [[ -e credentials.$user ]]; then
default_use_credential="Y"

printf "\e[1;31m[\e[0m\e[1;77m*\e[0m\e[1;31m]\e[0m\e[1;93m Credentials found for user !\e[0m\n"
read -p $'\e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;93m Use it?\e[0m\e[1;77m [Y/n]\e[0m ' use_credential

use_credential="${use_credential:-${default_use_credential}}"

if [[ $use_credential == *'Y'* || $use_credential == *'y'* ]]; then

while IFS= read -r line
do
  pass="$line"
done < credentials.$user
fi
fi

if [[ $pass == "" ]]; then
read -s -p $'\e[1;31m[\e[0m\e[1;77m*\e[0m\e[1;31m]\e[0m\e[1;93m Password: \e[0m' pass
echo $pass > credentials.$user
fi
...
hmac=$(echo -n "$data" | openssl dgst -sha256 -hmac "${ig_sig}" | cut -d " " -f2)
printf "\e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;93m Trying to follow celegram(hi dhrub and abhishek) %s ..." $celeb
...
printf "\e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;77m Sleeping 5 secs...\e[0m\n"
sleep 5
#unfollow
for celeb in $(cat celeb_id); do
data='{"_uuid":"'$guid'", "_uid":"'$username_id'", "user_id":"'$celeb'", "_csrftoken":"'$var2'"}'
hmac=$(echo -n "$data" | openssl dgst -sha256 -hmac "${ig_sig}" | cut -d " " -f2)
printf "\e[1;31m[\e[0m\e[1;77m+\e[0m\e[1;31m]\e[0m\e[1;93m Trying to unfollow celebgram(by dhruv and abhishek) %s ..." $celeb
